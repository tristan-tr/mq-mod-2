...
public static class DischargePatch
{
    ...
    [HarmonyPrefix]
    public static void rpcAddForcePrefix(PhysicsBody __instance, ref Vector3 impulse)
    {
        var status = __instance.GetComponent<WizardStatus>();
        if (status != null && status.dischargeCount > 0)
        {
            impulse *= 0.5f; // 50% knockback reduction
        }
    }
}
